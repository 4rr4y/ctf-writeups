...
We are given a mix of multiple challenges (about 5), but I was mainly concerned with getting the flag since weekends are precious. Here is the full `main.py` nonetheless:
...
While there are a few challenges, we can directly skip to challenge 5 for the flag since there are no pre-requisite checks. Here is the relevant code segment:
...
All we need to do is to upload a file named `kirby.txt` to get the flag, achievable via the following script:

```python
import requests

r = requests.post('http://34.130.180.82:59446/upload', files={'file':('kirby.txt', b'hello', 'text/plain')})
print(r.text)

# Output: <p class="error-message">wow thanks! very spicy fanfic. here&#39;s a gift: LITCTF{k1rBYl0L}</p>
```
